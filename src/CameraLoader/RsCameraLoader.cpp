...
void RsCameraLoader::init(std::string &serialNumber)
{
	...
	videoWriter_
			.open("../videos/RS" + serialNumber + ".mp4", cv::VideoWriter::fourcc('m', 'p', '4', 'v'), framerate_, cv::Size(imgWidth_, imgHeight_));
}
...
cv::Point3f RsCameraLoader::getCameraPosition(const cv::Point2f &graphCenter)
{
	...
	//邻近采样防止深度黑洞
	float position[3];
	cv::Rect2i imgRect = cv::Rect2i(0, 0, colorImg_.cols, colorImg_.rows);
