...
{
	cap_ = cv::VideoCapture(200 + devIndex);
//	cap_.set(CAP_PROP_FRAME_WIDTH,1280);
//	cap_.set(CAP_PROP_FRAME_HEIGHT,720);

	videoWriter_.open("../videos/Camera" + std::to_string(devIndex) + ".mp4", cv::VideoWriter::fourcc('m', 'p', '4', 'v'), framerate_,
	                  cv::Size(imgWidth_, imgHeight_));
}
